...
def search_results():
    ...
    edate = datetime.date(int(startdate[0]),int(startdate[1]),int(startdate[2]))
    results = mc.sentenceCount(keywords,
                               solr_filter=['tags_id_media:9139487'], split=True, split_start_date = "-".join(startdate), split_end_date="-".join(enddate))
